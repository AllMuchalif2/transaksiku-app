  ...
  Widget build(BuildContext context) {
    return Consumer<TransaksiProvider>(
      builder: (context, provider, child) {
        // START: Ambil dan filter data pemasukan dengan tambahan filter tahunan
        List<Transaksi> data;

        if (_filterMode == FilterMode.semua) {
          data = provider.getByJenis('pemasukan');
        } else if (_filterMode == FilterMode.bulanan) {
          data = provider
              .getByJenis('pemasukan')
              .where(
                (t) =>
          ...
          data = provider.getByJenisDanTanggal('pemasukan', _selectedDate);
        }

        int total = data.fold(0, (sum, item) => sum + item.jumlah);
        // END: Ambil dan filter data pemasukan dengan tambahan filter tahunan
        ...
        return Scaffold(
          ...
          // START: Ringkasan total pemasukan dengan format yang diperbaiki
          body: Column(
            children: [
              Container(
              ...
              // END: Ringkasan total pemasukan dengan format yang diperbaiki

              // START: List pemasukan dengan tombol edit dan hapus terpisah
              Expanded(
                child: data.isEmpty
                                  ...
                                  onPressed: () async {
                                    final confirm = await showDialog<bool>(
                                      ...
                                      builder: (_) => AlertDialog(
                                        ...
                                        content: Text(
                                          'Apakah Anda yakin ingin menghapus pemasukan "${t.nama}"?',
                                        ),
                                        actions: [
                                          TextButton(
                                            onPressed: () =>
              ...
              // END: List pemasukan dengan tombol edit dan hapus terpisah
            ],
          ),

          // START: Tombol tambah pemasukan
          floatingActionButton: FloatingActionButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const FormTransaksi(jenis: 'pemasukan'),
                ),
              );
            },
